    ...
    WHERE
        ...
        AND "aggregated_basket_items_@{eram_site_id}"."date" BETWEEN '@{date_start}' and '@{date_end}'
            and aggregated_basket_items_@{eram_site_id}.brand_id in (select brand_id from brands_company_accounts where company_account_id = @{eram_company_account_id})
                ...
                AND "aggregated_basket_items_@{eram_site_id}"."product_id" IN (
                SELECT
                    "products"."id"
                FROM
                    "products"
                WHERE id = ANY ((select product_ids from segments where segments.id = @{eram_segment_id})::bigint[]))
...
(select segments.name as segment, 'company' as level, retailer_categories.id, retailer_categories.name as category, count(distinct abi.basket_id) / (select baskets from basks limit 1)::float as cat_penetration
from basketss
...
(select segments.name as segment, 'total' as level, retailer_categories.id, retailer_categories.name as category, count(distinct abi.basket_id) / (select baskets from basks_tot limit 1)::float as cat_penetration
from basketss_tot
