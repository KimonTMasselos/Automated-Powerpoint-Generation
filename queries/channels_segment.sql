...
(select segments.name as segment, 'company' as level, source_medium.name as channel, count(distinct abm.basket_id)::float / (select * from basks) as perc
from basketss
...
group by segments.name, source_medium.name)

union

(select segments.name as segment, 'total' as level, source_medium.name as channel, count(distinct abm.basket_id)::float / (select * from basks_tot) as perc
from basketss_tot
