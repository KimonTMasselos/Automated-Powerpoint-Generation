...
bundles_data <- function(eram_company_account_id, date_start, date_end, eram_site_id, segment_id, source, type, con) {
  ...
  if (type == 'S') {
    ...
    query <- qq(read_file("../queries/bundles_segment.sql"))
  ...
  if (nrow(dt) >= 1) {
    ...
    basket_rules <- apriori(transactions, parameter = list(support = 0, confidence = 0.01, target = "rules", maxlen = 2))
    ...
    if (nrow(basket_analysis) > 1) {
      ...
      data %>%
        ...
        gtsave(filename = paste0("../output/", eram_company_account_id, "_", eram_site_id, "_", segment_id, "_10.png"), vwidth = 2000, zoom = 2)
