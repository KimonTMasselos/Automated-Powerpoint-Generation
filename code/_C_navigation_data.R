...
navigation_data <- function(eram_company_account_id, date_start, date_end, eram_site_id, segment_id, source, type, con) {
  ...
  if (type == 'S') {
    ...
    query <- qq(read_file("../queries/direct_category.sql"))
  ...
  if (nrow(dt) > 0) {
    ...
    data <- rbind(data, dt) %>% 
      mutate(sessions = as.numeric(sessions),
             page = str_trunc(page, 100, "right"))
    
    data %>% 
      select(Page = page, `Pageviews` = sessions, `Share of Assortment` = soa) %>%
      gt() %>%
      ...
      gtsave(filename = paste0("../output/", eram_company_account_id, "_", eram_site_id, "_", segment_id, "_04.png"), vwidth = 2000, zoom = 2)
